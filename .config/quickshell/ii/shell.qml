...
ShellRoot {
    ...
    // Independent of panelFamily: floats every window on demand, without
    // switching panel styles. Separate rule handle from Waffle's own float
    // rule so the two never fight over the same Hyprland-side variable.
    function toggleWindowedMode() {
        Config.options.windowedMode = !Config.options.windowedMode
    }

    Process {
        id: windowedFloatRuleToggle
        command: ["hyprctl", "eval",
            (Config.options.windowedMode
                ? ("if not _windowedFloatRule then\n" +
                   "  _windowedFloatRule = hl.window_rule({ name = 'windowed-mode-float-all', match = { class = '.*' }, float = true, size = {'(monitor_w*0.6)', '(monitor_h*0.6)'} })\n" +
                   "else\n" +
                   "  _windowedFloatRule:set_enabled(true)\n" +
                   "end")
                : "if _windowedFloatRule then _windowedFloatRule:set_enabled(false) end")
        ]
    }

    // Full transition — includes the one-shot float/tile of pre-existing
    // windows, which should only fire when windowedMode actually changes.
    function applyWindowedModeTransition() {
        windowedFloatRuleToggle.running = true
        if (Config.options.windowedMode) {
    ...
    Connections {
        ...
        function onWindowedModeChanged() {
            // Same Config.ready gating as panelFamily above: skip the one-shot
            // float/tile pass on a plain reload where windowedMode was already
            // persisted as true/false, only react to an actual toggle.
            if (Config.ready) {
                root.applyWindowedModeTransition()
            }
        }
    }
    ...
    IpcHandler {
        ...
        function toggle(): void {
            root.toggleWindowedMode()
        }
    }

    GlobalShortcut {
        name: "windowedModeToggle"
        description: "Toggles floating (windowed) mode for all windows, independent of panel family"

        onPressed: root.toggleWindowedMode()
    }

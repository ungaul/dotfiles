...
Singleton {
	id: root;
	property int _playbackVersion: 0;
	property list<MprisPlayer> players: {
		_playbackVersion;
		return Mpris.players.values.filter(player => isRealPlayer(player) && player.playbackState !== MprisPlaybackState.Stopped);
	}
	property MprisPlayer trackedPlayer: null;
	property MprisPlayer activePlayer: {
		_playbackVersion;
		if (trackedPlayer && trackedPlayer.playbackState !== MprisPlaybackState.Stopped)
	...
	Instantiator {
		...
		Connections {
			...
			Component.onCompleted: {
				if (modelData.playbackState !== MprisPlaybackState.Stopped) {
			...
			Component.onDestruction: {
				if (root.trackedPlayer == null || !root.trackedPlayer.isPlaying) {
					...
					if (root.trackedPlayer == null || root.trackedPlayer.playbackState === MprisPlaybackState.Stopped) {

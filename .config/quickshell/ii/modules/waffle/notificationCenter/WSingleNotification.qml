...
MouseArea {
    ...
    // Some senders include a no-label "default" action meant only for clicking the
    // notification body, not for a visible button — filter those out so we don't
    // render an empty-looking action button.
    readonly property var visibleActions: notification.actions.filter(a => a.text)

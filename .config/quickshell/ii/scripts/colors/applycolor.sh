...
apply_ghostty() {
  ...
  local GHOSTTY_THEME="$XDG_CONFIG_HOME/ghostty/themes/matugen"
  cp "$SCRIPT_DIR/terminal/ghostty-theme.conf" "$GHOSTTY_THEME"
  for i in "${!colorlist[@]}"; do
    sed -i "s/${colorlist[$i]} #/${colorvalues[$i]#\#}/g" "$GHOSTTY_THEME"
  done
}
